...
namespace Leap.Forward.Idle.CodeGen
{
    ...
    public class IdleViewModelGenerator : IIncrementalGenerator
    {
        ...
        {{
            ...
            sourceBuilder.AppendLine("        /// Binds this data source to the specified UI document and attaches event handlers to upgrade buttons within the");
            sourceBuilder.AppendLine("        /// document.");
            sourceBuilder.AppendLine("        /// </summary>");
            sourceBuilder.AppendLine("        /// <remarks>This method sets the document's root visual element's data source to this instance and");
            sourceBuilder.AppendLine("        /// subscribes to the 'clicked' events of the buttons if necessary. If any required button is");
            sourceBuilder.AppendLine("        /// missing, an error is logged. The method does not throw exceptions for missing elements.</remarks>");
            sourceBuilder.AppendLine("        /// <param name=\"document\">The UI document to bind to.</param>");
            ...
            if (el_{method.Name} != null) 
                ...
                UnityEngine.Debug.LogError($""{className}.BindTo() Could not find Button with name '{elementName}' in document {{document.name}}"");");
                }
            }
